...
The `/init` command is a powerful feature in Otto that automatically analyzes your project and creates a tailored `EASYCODE.md` file to provide context for future AI interactions.
...
Simply type `/init` in the Otto CLI:
...
- **Special Considerations**: Otto specific configurations and AI assistance patterns
...
The generated `EASYCODE.md` file becomes part of Otto's memory system, providing:
...
This command is inspired by the Gemini CLI's `/init` command but adapted specifically for Otto's workflow and `EASYCODE.md` context system.