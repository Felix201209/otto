# Otto Hooks 安装与配置指南（跨平台）

## 创建 hooks 目录

**Windows：**
```cmd
mkdir "%USERPROFILE%\.easycode-user\hooks"
```

**macOS/Linux：**
```bash
mkdir -p "$HOME/.easycode-user/hooks"
```
...
**Windows：**
```cmd
copy "D:\Projects\easycode\Otto\.easycode\hooks\hook-template.bat" "%USERPROFILE%\.easycode-user\hooks\test-hook-logger.bat"
```

**macOS/Linux：**
```bash
cp "/Users/yourname/Projects/easycode/Otto/.easycode/hooks/hook-template.sh" "$HOME/.easycode-user/hooks/test-hook-logger.sh"
chmod +x "$HOME/.easycode-user/hooks/test-hook-logger.sh"
```
    ...
    "AfterTool": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "cmd /c %USERPROFILE%\\.easycode-user\\hooks\\test-hook-logger.bat",
            "timeout": 5000
          }
        ]
      }
    ...
    "AfterTool": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "$HOME/.easycode-user/hooks/test-hook-logger.sh",
            "timeout": 5000
          }
        ]
      }
...
**Windows：**
```cmd
type "%USERPROFILE%\.easycode-user\hooks\hook_log.txt"
```

**macOS/Linux：**
```bash
cat "$HOME/.easycode-user/hooks/hook_log.txt"
```
...
Otto 支持在系统的关键生命周期节点触发钩子。以下是所有支持的事件类型：
...
### 环境变量
所有钩子都会收到以下环境变量：
- `EASYCODE_SESSION_ID`: 当前会话 ID
- `EASYCODE_HOOK_EVENT`: 触发的事件名称（如 `BeforeModel`）
- `EASYCODE_TIMESTAMP`: 触发时间戳
...
某些钩子允许脚本通过输出特定的 JSON 来修改 Otto 的行为。
...
| 创建目录 | `mkdir "%USERPROFILE%\.easycode-user\hooks"` | `mkdir -p "$HOME/.easycode-user/hooks"` |
| 拷贝模板 | `copy src dst` | `cp src dst` |
| 权限设置 | 无需 | `chmod +x` |
| 查看日志 | `type file` | `cat file` |
| 命令写法 | `"cmd /c %USERPROFILE%\\.easycode-user\\hooks\\script.bat"` | `"$HOME/.easycode-user/hooks/script.sh"` |