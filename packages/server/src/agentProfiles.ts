...
/**
 * 用户确认生成的 profile.json 也是本机白名单的一部分，但不能覆盖随包专家。
 * 这里只接收收窄后的字段；部门/edition 等权限字段不信任文件输入。
 */
export function resolveAutoGeneratedAgentProfile(
  id: string | undefined,
  profiles: readonly AutoSkillProfileData[],
): ServerAgentProfile | undefined {
  if (!id || !/^[a-z0-9-]+$/.test(id) || profileById.has(id)) return undefined;
  const source = profiles.find((profile) => profile.id === id);
  if (!source) return undefined;
  const name = source.name.trim().slice(0, 80);
  const systemPrompt = source.systemPrompt.trim().slice(0, 40_000);
  ...
  const skills = source.skills
    ...
    .slice(0, 16);
  const profile: ServerAgentProfile = {
    id,
    name,
    scope: 'base',
    edition: 'both',
    skills,
    systemPrompt: `${systemPrompt}\n\n身份规则：你的当前身份是「${name}」。如果用户问“你是谁”或询问你的能力，用一句话回答你是「${name}」并概括上文定义的职责；不得自称为其他专家。`,
  };
  return {
    ...profile,
    welcomeMessage: buildWelcomeMessage(profile),
...
export function resolveAgentProfile(id: string | undefined): ServerAgentProfile | undefined {
  ...
  const builtin = profileById.get(id);
  if (builtin) return builtin;
  try {
    return resolveAutoGeneratedAgentProfile(id, loadAutoGeneratedProfiles());
  } catch {
    return undefined;
  }
}
