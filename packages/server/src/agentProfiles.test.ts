...
import {
  ...
  resolveAutoGeneratedAgentProfile,
  resolveAgentProfile,
...
describe('服务端 Agent profile 白名单', () => {
  ...
  it('只接受用户确认生成且不覆盖内置 id 的本地专家 profile', () => {
    const generated = [{
      ...
      scope: 'personal',
      department: null,
      skills: ['auto-weekly-report'],
      systemPrompt: '你是周报专家，只依据真实工作记录生成结构化周报。',
    }];

    expect(resolveAutoGeneratedAgentProfile('auto-weekly-report', generated)).toMatchObject({
      id: 'auto-weekly-report',
      name: '周报专家',
      edition: 'both',
      skills: ['auto-weekly-report'],
      systemPrompt: expect.stringContaining('当前身份是「周报专家」'),
      welcomeMessage: expect.stringContaining('周报专家'),
    });
    expect(resolveAutoGeneratedAgentProfile('ppt', [{
      ...generated[0],
      id: 'ppt',
    }])).toBeUndefined();
    expect(resolveAutoGeneratedAgentProfile('../escape', [{
      ...generated[0],
      id: '../escape',
    }])).toBeUndefined();
  });
