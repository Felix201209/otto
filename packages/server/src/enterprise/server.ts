...
async function loadWorkspaceWithFeedback(){showError('pageError','');try{await loadWorkspace()}catch(error){showError('pageError','后台数据加载失败：'+(error&&error.message?error.message:'请稍后重试'))}}
...
$('loginForm').addEventListener('submit',async event=>{event.preventDefault();showError('loginError','');setLoginPending(true);try{const data=await api('/enterprise/auth/admin/login',{method:'POST',body:JSON.stringify({identifier:$('username').value.trim(),password:$('password').value})});token=data.token;currentAdmin=data.account;sessionStorage.setItem(KEY,token);$('password').value='';setPasswordVisible(false);showAdmin();await loadWorkspaceWithFeedback()}catch(error){token='';sessionStorage.removeItem(KEY);$('password').value='';setPasswordVisible(false);showLogin(error&&error.message?error.message:'登录失败，请稍后重试');$('password').focus()}finally{setLoginPending(false)}});
...
(async()=>{if(!token)return showLogin('');try{const data=await api('/enterprise/auth/me');if(!data.account.isAdmin)throw new Error('该账号没有管理员权限');currentAdmin=data.account}catch{token='';sessionStorage.removeItem(KEY);showLogin('登录已失效，请重新登录');return}showAdmin();await loadWorkspaceWithFeedback()})();
</script></body></html>`;
}
