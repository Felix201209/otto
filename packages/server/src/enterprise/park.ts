 ...
 *
 * Park / Industrial Park - park admin configures invite codes, service specialists,
 * and repair routing. Enterprises join a park via invite code.
 */

import { getDB, getOrganization } from './db.js';
...
function toPark(row: ParkRow): Park {
  ...
  try { adminUserIds = JSON.parse(row.admin_user_ids); } catch { /* keep [] */ }
  return {
    id: row.id,
    name: row.name,
    address: row.address,
...
export function createPark(input: {
  ...
  if (!name || name.length > 120) throw new Error('园区名称不能为空且不能超过 120 个字符');
...
function generateParkInviteCode(): string {
  let code = '';
  for (let i = 0; i < 8; i++) {
    code += PARK_INVITE_CHARS[Math.floor(Math.random() * PARK_INVITE_CHARS.length)];
  }
  return code;
}
...
export function validateInviteCode(code: string): {
  ...
  if (row.used_count >= row.max_uses) return { valid: false, error: '邀请码已达使用上限' };
...
export function useInviteCode(code: string, enterpriseId: string): {
  success: boolean;
  parkId?: string;
  error?: string;
} {
...
function toSpecialist(row: ParkServiceSpecialistRow): ParkServiceSpecialist {
  ...
  try { serviceTypes = JSON.parse(row.service_types); } catch { /* keep [] */ }
  return {
    parkId: row.park_id,
    userId: row.user_id,
    serviceTypes,
...
export function assignSpecialist(input: {
  parkId: string;
  userId: string;
  serviceTypes: string[];
...
export function routeServiceRequest(requestId: string): ParkServiceRequest | null {
  ...
  const matching = specialists.filter((s) => {
    let types: string[] = [];
    try { types = JSON.parse(s.service_types); } catch { /* */ }
...
export function getParkServiceRequests(
  parkId: string,
  status?: string,
): ParkServiceRequest[] {
  ensureParkSchema();
