...
describe('loadExtensions', () => {
  ...
  it('should load context files based on priority order', async () => {
    const workspaceExtensionsDir = path.join(
      tempWorkspaceDir,
      EXTENSIONS_DIRECTORY_NAME,
    );
  ...
  it('should fallback to next priority when higher priority files do not exist', async () => {
    const workspaceExtensionsDir = path.join(
      tempWorkspaceDir,
      EXTENSIONS_DIRECTORY_NAME,
    );
    ...
    // Only create GEMINI.md, AGENTS.md and OTTO.md don't exist
    fs.writeFileSync(path.join(extDir, 'GEMINI.md'), 'gemini content');
    fs.writeFileSync(path.join(extDir, 'CLAUDE.md'), 'claude content');
