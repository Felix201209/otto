...
> 本知识库包含 Otto CLI 的所有命令和功能详细说明，供智能帮助系统使用。
...
**位置选择：**
- 全局命令：`~/.otto-user/commands/`（所有项目可用）
- 项目命令：`<项目根目录>/.otto/commands/`（仅当前项目）

**文件格式示例：**
```toml
# 文件：~/.otto-user/commands/git/commit.toml
# 命令名：/git:commit
...
**位置选择：**
- 全局配置：`~/.otto-user/settings.json`（所有项目可用）
- 项目配置：`<项目根目录>/.otto/settings.json`（仅当前项目）
...
**A:** 是的，Otto 也支持 VSCode 扩展！
你可以在 VSCode 扩展市场中搜索 **Otto for VSCode** 在线安装，即可在 VSCode 中使用图形化界面版本。

**安装方式：**
1. 打开 VSCode
2. 进入扩展市场（快捷键：`Ctrl+Shift+X` 或 `Cmd+Shift+X`）
3. 搜索 `Otto for VSCode`
4. 点击安装
...
**A:** Otto 支持配置 OpenAI 兼容格式和 Anthropic Claude API 格式的自定义模型。

#### 📝 配置文件位置

自定义模型配置保存在：`~/.otto-user/custom-models.json`
...
编辑 `~/.otto-user/custom-models.json`：
...
- **重启生效**：修改配置后需要重启 Otto
- **Token 计费**：自定义模型不消耗 Otto 积分，直接向提供商付费
...
**A:** 使用 `/feishu` 子命令系列，可以将 Otto 直接作为一个智能群聊或私聊 Bot 接入到你的飞书工作区：
1. **启动配置：** 输入 `/feishu`，选择 QR 扫码自动建应用或手动配置。
2. **凭据安全：** 凭证经过 AES-256-GCM 高强度加密并存放在 `~/.otto-user/` 全局目录下，自动采用 0o600 权限锁闭。
3. **日常管控：**
...
### `/memory` - 记忆管理
管理 AI 的指令上下文（从 `OTTO.md` 文件加载）。

**子命令：**
- `add <text>` - 添加文本到 AI 记忆
- `show` - 显示当前加载的完整分层记忆
- `refresh` - 重新加载所有  `OTTO.md` 文件
...
**记忆文件层次（优先级递增）：**
1. 全局：`~/.otto-user/OTTO.md`
2. 项目根及父目录：`<项目路径>/OTTO.md`
3. 子目录：项目内各子目录的 `OTTO.md`
...
### `/config` - 统一配置菜单
打开交互式设置面板，让您可以直观地配置 Otto 的各种设置。常用的设置如主题、模型、工作方式等都在这里了。

**用法：** `/config [子命令]`

**功能：**
- **交互式面板：** 直接输入 `/config` 回车，打开可视化菜单。
- **常用设置：** 面板集成了主题、编辑器、模型切换、Vim 模式、工作方式、自动批准、健康使用提醒及语言偏好。
- **快捷操作：** 支持键盘上下移动、回车进入子菜单、`Esc` 返回或关闭。

**子命令：**
- `/config theme` - 打开主题选择
- `/config model` - 打开模型选择
- `/config agent-style` - 切换 Otto 工作方式
...
**持久化：** 偏好保存到 `~/.otto-user/settings.json`
...
### `/agent-style` - Otto 工作方式
切换 Otto 的表达和执行方式，用日常场景选择即可，不需要了解其他 AI 工具。
...
| 风格 | 特点 | 适合场景 |
|-----|------|---------|
| **日常对话** 🧠 | 自然表达、解释陌生概念 | 日常问答、学习、需要理解过程 |
| **快速执行** ⚡ | 少量过程说明，直接交付结果 | 熟练场景、重复任务、追求效率 |
| **工作代码** 🎯 | 简短同步进度，遵循代码规范 | 日常开发、代码修改和排查 |
| **工程交付** 🚀 | 任务拆分、严格验证 | 多步骤开发、重构和正式交付 |
| **简洁开发** ⌨️ | 极简响应、直接行动 | 高频命令行开发交互 |
| **企业办公** 💎 | 文档、会议、日程、表格和调研 | 企业员工的日常办公工作 |
| **协作推进** 🌊 | 独立执行与清晰解释并重 | 需要边做边沟通的复杂工作 |

**示例：**
```
/agent-style          # 查看当前状态
/agent-style code     # 切换到工作代码
...
### `/feishu` - 飞书机器人管理
管理 Otto 与飞书 (Feishu) / Lark 的机器人长连接及接入。
...
**位置：**
- 全局：`~/.otto-user/commands/`
- 项目：`<项目根>/.otto/commands/`

#### 文件格式：TOML（`.toml` 扩展名）

#### 命名：文件路径决定命令名
- `~/.otto-user/commands/test.toml` → `/test`
- `<项目>/.otto/commands/git/commit.toml` → `/git:commit`
...
**示例：**
```toml
# ~/.otto-user/commands/changelog.toml
...
### 配置层次（优先级递增）
1. **默认值**：硬编码默认值
2. **用户设置**：`~/.otto-user/settings.json`（全局）
3. **项目设置**：`<项目根>/.otto/settings.json`（项目专用）
...
- `--output-format <format>`：非交互模式的输出格式
  ...
  - 示例：`otto --output-format stream-json --yolo "你的prompt内容"`
- `--yolo`：自动批准所有工具调用
...
```bash
# 标准输出
otto --prompt "解释 main.py" -a

# JSONL 格式（一行一个 JSON 对象）
otto --output-format stream-json --yolo "分析代码并生成改进建议" @src/

# 与其他工具组合
otto --output-format stream-json --yolo "生成代码" @schema.json | jq '.content'
```
...
### 自定义沙箱
在项目根创建 `.otto/sandbox.Dockerfile`：
```dockerfile
...
- `strict`：严格模式
- `<profile_name>`：自定义 profile（`.otto/sandbox-macos-<profile_name>.sb`）
...
### 场景 4：配置 MCP 服务器
编辑 `.otto/settings.json` 或 `~/.otto-user/settings.json`：
```json
{
  "mcpServers": {
    "database": {
...
### 场景 5：创建自定义命令
创建 `~/.otto-user/commands/docs/generate.toml`：
```toml
...
### 问题：自定义命令不显示
**检查：**
1. 文件位置：`~/.otto-user/commands/` 或 `<项目>/.otto/commands/`
2. 文件扩展名：必须是 `.toml`
...
这份知识库涵盖了 Otto CLI 的所有核心功能。如果你有任何问题，请随时询问！
...
**A:** 使用 `otto checkpoint clean` 命令清理所有历史检查点。

**命令用法：**
```bash
# 清理所有 checkpoint 历史（会提示确认）
otto checkpoint clean

# 预览将要删除的内容（不实际删除）
otto checkpoint clean --dry-run

# 跳过确认直接删除
otto checkpoint clean --force
```

**别名：** 可以使用 `otto cp clean` 作为简写。
...
📊 Checkpoint History Summary:
   Projects: 73
   Total Size: 4.18 GB
   Location: C:\Users\username\.otto-user\history
...
Skills 系统数据存储在：
- **目录：** `~/.otto-user/skills/` 和 `~/.otto-user/marketplace/`
- **配置：** `~/.otto-user/skills/settings.json`
- **已安装记录：** `~/.otto-user/skills/installed_plugins.json`
- **备份：** `~/.otto-user/skills/backups/`
...
**A:** Otto 提供了强大的 Hooks 系统，允许你用任何脚本语言（Python、Bash、PowerShell、Batch 等）在关键操作点插入自定义脚本来监控、控制和修改系统行为。
...
```
配置：`"command": "bash .otto/hooks/security.sh"`
...
sys.exit(0)
```
配置：`"command": "python .otto/hooks/security.py"`
...
```
配置：`"command": "powershell -ExecutionPolicy Bypass .otto/hooks/security.ps1"`

**第 3 步：配置到 `.otto/settings.json`**
```json
{
  "hooks": {
    "BeforeTool": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "python .otto/hooks/security.py",
            "timeout": 5000
          }
        ]
      }
...
**第 4 步：测试**
```bash
echo '{"tool_name":"delete_file"}' | python .otto/hooks/security.py
...
```json
{
  "hooks": {
    "BeforeTool": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "python .otto/hooks/hook.py",
...
Otto 现在集成了强大的 **LSP (Language Server Protocol)** 能力。这意味着 AI 不再仅仅是通过“搜索字符串”来猜你的代码，而是像真正的 IDE 一样，能够完全“读懂”代码的语义。

### 什么是 LSP？
**LSP** 是由微软、Google 和 RedHat 共同推出的行业标准。它将代码分析逻辑与编辑器分离。通过 LSP，Otto 可以调用各语言官方最专业的分析引擎（如 `typescript-language-server`, `pyright`, `rust-analyzer` 等），为 AI 提供极致的代码感知力。
...
- **自动安装**：如果你的电脑没装对应的 LSP 服务端，Otto 会**自动在后台下载并安装**到隔离目录。
...
### 核心功能与工具集
Otto 提供了一套完整的语义化工具，帮助 AI 实现“手术刀”级的代码操作：
