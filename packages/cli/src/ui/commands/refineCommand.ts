 ...
 * 清理润色输出中的无关内容
 *
 * 优先尝试提取 <otto-refine-prompt> 标签内的内容
 */
function cleanRefineOutput(text: string): string {
  let cleaned = text.trim();

  // 1. 尝试提取 <otto-refine-prompt>...</otto-refine-prompt> 标签内的内容
  const tagMatch = cleaned.match(/<otto-refine-prompt>([\s\S]*?)<\/otto-refine-prompt>/);
