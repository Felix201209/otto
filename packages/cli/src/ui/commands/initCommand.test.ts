  ...
  it('should proceed with init when OTTO.md is empty (0 bytes)', async () => {
    ...
    expect(result).toEqual(
      expect.objectContaining({
        type: 'submit_prompt',
        content: expect.stringContaining('You are a Otto AI assistant'),
      }),
    );
  });

  it('should create OTTO.md and submit a prompt if it does not exist', async () => {
    ...
    expect(mockContext.ui.addItem).toHaveBeenCalledWith(
      {
        type: 'info',
        text: 'Creating OTTO.md... Now analyzing the project to populate it.',
      },
      expect.any(Number),
    );
