...
import { unescapePath } from 'otto-core';
    ...
    if (logLine.length === 0) {
      ...
      while (currentPosInLogLine < codePointsInLogLine.length) {
        ...
        for (let i = currentPosInLogLine; i < codePointsInLogLine.length; i++) {
          ...
          const charVisualWidth = getCachedCharWidth(char); // 🚀 使用缓存的字符宽度计算
...
export function useTextBuffer({
  ...
  const linesHash = useMemo(() => {
    // 对于短文本，使用长度作为简单hash；对于长文本，使用更复杂的hash
    if (lines.length === 1 && lines[0].length < 100) {
      return lines[0].length.toString();
    }
    return `${lines.length}-${text.length}-${lines[0]?.slice(0, 10) || ''}`;
  }, [lines, text.length]);

  const visualLayout = useMemo(
    () => {
      // 🚀 性能优化：对于不需要换行的短文本跳过复杂布局计算
      ...
      if (totalTextLength < 500 && lines.length === 1 && !text.includes('\n')) {
        ...
        if (visualWidth <= state.viewportWidth) {
          return {
            visualLines: lines,
            visualCursor: [cursorRow, cursorCol] as [number, number],
...
export interface TextBuffer {
  ...
  /**
   * Gets the current cursor position as an absolute text offset.
   */
