  ...
  constructor(private readonly config: Config) {
    super(
      ...
      [
        ...
        '- Create wiki node with content (TWO-STEP REQUIRED):',
        '  Step 1 — Create doc first: command="docs +create" args=["--api-version", "v2", "--title", "Title", "--content", "@<relative-path>", "--doc-format", "markdown"]',
        '  Step 2 — Link doc to wiki: command="wiki +node-create" args=["--space-id", "<id>", "--title", "Title", "--obj-token", "<obj_token_from_step1>"]',
        '  CRITICAL: Never run wiki +node-create without --obj-token. Doing so creates an empty placeholder document in the wiki that cannot be deleted via API. Always create the document first with docs +create, then link it with --obj-token.',
