...
describe('FeishuProjectCollabTool', () => {
  ...
  it('renders project collaboration tables and acceptance nodes', async () => {
    const result = await tool.execute({
      ...
      project_goal: 'Ship enterprise collaboration MVP',
      collaborators: [{ name: 'Alice', role: 'PM', responsibility: 'Acceptance criteria' }],
      acceptance_nodes: [{ name: 'MVP Review', due: '2026-08-01', standard: 'All tables and reminders ready', owner: 'Alice' }],
  ...
  it('requires progress content for progress sync', async () => {
    ...
    expect(result.llmContent).toContain('progress_sync requires progress_content');
  });

  it('renders reminder plan before acceptance nodes', async () => {
    const result = await tool.execute({
      action: 'reminder_plan',
      project_name: 'Launch Plan',
      acceptance_nodes: [{ name: 'Final Acceptance', due: '2026-08-02', standard: 'Signed off', owner: 'Bob' }],
    }, new AbortController().signal);
