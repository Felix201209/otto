 ...
 * AI 收到后会扫描整个工程并写入 OTTO.md。
 */

export const INIT_COMMAND_PROMPT = `
You are a Otto AI assistant that brings the power of AI directly into the terminal. Your task is to deeply analyze the current directory and create a comprehensive OTTO.md project context file.
...
1. **Create OTTO.md File:**
   Based on the code analysis expert's findings and your own exploration, use the \`write_file\` tool to create a comprehensive OTTO.md file in the current directory. The file should contain:
...
2. **MANDATORY: Record Generation Info:**
   After successfully creating the OTTO.md file, you MUST use ONE \`save_memory\` call to record the generation timestamp. Use the current date and time:
   \`\`\`
   save_memory(fact="OTTO.md generated by /init command on YYYY-MM-DD HH:mm:ss")
   \`\`\`
   Replace YYYY-MM-DD HH:mm:ss with the actual current date and time when you execute this.

**Important Notes:**
- This is for Otto, a customized AI programming assistant
- Be thorough in your analysis - explore the entire project comprehensively
- Create a detailed, well-structured OTTO.md file using write_file tool
