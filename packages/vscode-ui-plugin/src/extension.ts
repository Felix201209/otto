...
import {
  ...
  generateLocalPpt,
  isStructuredPptStoryboard,
} from './services/localPptGeneration';
...
export async function activate(context: vscode.ExtensionContext) {
  ...
  try {
    ...
    if (needsLegacyMigration(migrateTarget)) {
      ...
      const startTitle = isZh
        ? '🎉 Otto 现已升级为 Otto'
        : '🎉 Otto is now upgraded to Otto';
      const startDetail = isZh
        ? '即将开始迁移您的历史数据与配置到 Otto。根据数据大小，这可能需要数秒到数十秒，期间请勿关闭 VS Code。点击「确定」开始。'
        : 'Your legacy data and settings will now be migrated to Otto. Depending on the data size this may take a few to tens of seconds — please do not close VS Code. Click "OK" to begin.';
      ...
      migrateLegacyDirectories(migrateTarget, (type) => {
        console.log(`[Migration] Migrating legacy ${type}-level directory to Otto...`);
      });

      // 3) 迁移【后】完成提示。
      const doneTitle = isZh ? '✅ 迁移完成' : '✅ Migration complete';
      const doneDetail = isZh
        ? '您的历史数据与配置（settings、custom-models、commands 等）已成功迁移到 Otto，可直接继续使用。'
        : 'Your legacy data and settings (settings, custom-models, commands, etc.) have been migrated to Otto. You can continue right away.';
  ...
  console.log('=== Otto AI Assistant: Starting activation ===');
  ...
  try {
    ...
    try {
      const { setCliVersion } = require('otto-core');
    ...
    const outputChannel = vscode.window.createOutputChannel('Otto AI Assistant');
    ...
    logger.info('Otto AI Assistant is activating...');
    logger.info(`📁 Log file location: ${logger.getLogFilePath()}`);
    logger.info(`📁 Extension path: ${context.extensionPath}`);

    vscode.window.showInformationMessage('Otto AI Assistant is activating...');
    startupOptimizer.endPhase();
    ...
    logger.info('Otto AI Assistant activated successfully');
    console.log('=== Otto AI Assistant: Activation completed ===');
    vscode.window.showInformationMessage('Otto AI Assistant activated successfully!');
    ...
    console.error('=== Otto AI Assistant: Activation failed ===', error);
    if (logger) {
      logger.error('Failed to activate extension', error instanceof Error ? error : undefined);
    }
    const message = error instanceof Error ? error.message : String(error);
    vscode.window.showErrorMessage(`Failed to activate Otto AI Assistant: ${message}`);
    throw error; // Re-throw to ensure VS Code knows activation failed
  }
}

export async function deactivate(): Promise<void> {
  logger?.info('Otto AI Assistant is deactivating...');
  ...
  try {
    ...
    logger?.info('Otto AI Assistant deactivated successfully');
...
function setupBasicMessageHandlers() {
  ...
  communicationService.onProjectSettingsUpdate(async (data) => {
    try {
      ...
      if (workspaceRoot) {
        const settingsDir = path.join(workspaceRoot, '.otto');
  ...
  communicationService.onProjectSettingsRequest(async () => {
    try {
      ...
      const workspaceRoot = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
      if (workspaceRoot) {
        const settingsPath = path.join(workspaceRoot, '.otto', 'settings.json');
        if (fs.existsSync(settingsPath)) {
          try {
...
async function handleBackgroundTaskComplete(
  ...
  try {
    ...
    if (status === 'completed') {
      notificationText = `[Otto - SYSTEM NOTIFICATION] Background task completed (Task ID: ${shortId}). Exit code: ${task.exitCode ?? 'unknown'}. Output:\n${outputPreview}`;
    } else if (status === 'failed') {
      notificationText = `[Otto - SYSTEM NOTIFICATION] Background task failed (Task ID: ${shortId}). Command: ${task.command}. Error: ${task.error || 'Unknown error'}. Output:\n${outputPreview}`;
    } else if (status === 'cancelled') {
      notificationText = `[Otto - SYSTEM NOTIFICATION] Background task killed by user (Task ID: ${shortId}). Command: ${task.command}. Output before kill:\n${outputPreview}`;
    }
    ...
    if (flowState.isProcessing) {
      ...
      const triggerMessage = {
        ...
        sessionId,
        content: [{ type: 'text' as const, value: '[Otto - SYSTEM NOTIFICATION] Background tasks have completed. Please review the results above and continue.' }],
        timestamp: Date.now(),
        type: 'user' as const,
      };
...
async function processPendingBackgroundNotifications(sessionId: string) {
  ...
  const triggerMessage = {
    id: `bg-trigger-${Date.now()}`,
    sessionId,
    content: [{ type: 'text' as const, value: '[Otto - SYSTEM NOTIFICATION] Background tasks have completed while you were busy. Please review the results above if necessary, and continue.' }],
    timestamp: Date.now(),
    type: 'user' as const,
  };
...
function setupBackgroundTaskHandlers() {
  // 延迟导入以避免循环依赖
  import('otto-core').then(({ getBackgroundTaskManager }) => {
    ...
    communicationService.onBackgroundTaskMoveToBackground(async (data) => {
      try {
        ...
        const { getBackgroundModeSignal } = await import('otto-core');
...
function setupLoginHandlers() {
      ...
      if (isCursor) {
        ...
        try {
          ...
          const action = await vscode.window.showInformationMessage(
            'Extension page opened in your browser. You can also search for "Otto" in Extensions (Ctrl+Shift+X).',
  ...
  communicationService.onSetCurrentModel(async (payload) => {
    try {
      ...
      const { ProxyAuthManager, tokenLimit } = require('otto-core');
      const proxyAuthManager = ProxyAuthManager.getInstance();
    ...
    try {
      ...
      await vscode.window.withProgress({
        ...
        if (switchResult.compressionInfo) {
          ...
          // 🎯 更新前端的 tokenUsage 显示
          const { tokenLimit } = require('otto-core');
   ...
   */
  const hotReloadCustomModelsToAllSessions = async (models: import('otto-core').CustomModelConfig[]) => {
    try {
...
<otto-refine-prompt>enhanced prompt goes here</otto-refine-prompt>
### END RESPONSE ###
...
Here is my original instruction:
 ...
 ${originalText}`;
    ...
    try {
      ...
      // 🎯 清理AI响应，提取 <otto-refine-prompt> 标签内的内容
      let cleanedText = refinedText.trim();

      // 尝试提取 <otto-refine-prompt>...</otto-refine-prompt> 标签内的内容
      const tagMatch = cleanedText.match(/<otto-refine-prompt>([\s\S]*?)<\/otto-refine-prompt>/);
...
function setupSlashCommandHandlers() {
  ...
  communicationService.addMessageHandler('execute_custom_slash_command', async (payload: any) => {
    try {
      ...
      // ─────────────────────────────────────────────────────────────
      // /init 内置命令的特殊预处理：在 workspace 创建空 OTTO.md，再走 prompt 分支
      // ─────────────────────────────────────────────────────────────
      if (command.kind === 'built-in' && command.name === 'init') {
        ...
        try {
          ...
          if (fs.existsSync(ottoMdPath)) {
            ...
            if (stats.size === 0) {
              ...
              communicationService.sendMessage({
                ...
                payload: {
                  success: false,
                  error: `OTTO.md already exists (${sizeKB}KB). Delete or rename it first if you want to regenerate via /init.`,
                },
              });
              return;
            }
          ...
          logger.error('Failed to prepare OTTO.md for /init', initErr instanceof Error ? initErr : undefined);
          communicationService.sendMessage({
            type: 'slash_command_result',
            payload: {
              success: false,
              error: `Failed to prepare OTTO.md: ${initErr instanceof Error ? initErr.message : String(initErr)}`,
            },
          });
          return;
        }
...
function setupMultiSessionHandlers() {
  ...
  communicationService.onNanoBananaUpload(async (payload) => {
    try {
      ...
      const { ImageGeneratorAdapter } = await import('otto-core');
  ...
  communicationService.onPPTGenerate(async (payload) => {
    try {
      ...
      let storyboard = payload.outline;
      if (!isStructuredPptStoryboard(storyboard, payload.pageCount)) {
        const response = await client.getContentGenerator().generateContent(
          {
            model: coreConfig.getModel(),
            contents: [{
              role: 'user',
              parts: [{ text: buildPptStoryboardPrompt(payload) }],
            }],
            config: { abortSignal: new AbortController().signal },
          },
          SceneType.CHAT_CONVERSATION,
        );
        storyboard = response.candidates?.[0]?.content?.parts
          ?.map((part) => part.text || '')
          .join('')
          .trim() || '';
        if (!storyboard) throw new Error('当前模型没有返回可渲染的 PPT 故事板');
        logger.info('PPT storyboard generated by current session model');
      }

      const result = await generateLocalPpt(
        { ...payload, outline: storyboard },
        coreConfig,
  ...
  communicationService.onPPTOptimizeOutline(async (payload) => {
    try {
      ...
      const optimizePrompt = buildPptStoryboardPrompt({
        topic: payload.topic,
        pageCount: payload.pageCount,
        style: [payload.style, payload.colorScheme].filter(Boolean).join('；'),
  ...
  const commands = [
    vscode.commands.registerCommand('otto.openAIAssistant', async () => {
      ...
      try {
        ...
        vscode.window.showErrorMessage('Failed to open Otto Assistant');
      }
    }),
...
💡 命中率高说明缓存策略有效，减少了 API 调用。`;
  ...
  console.log(`Otto: Registered ${commands.length} commands`);
}
...
async function initializeInlineCompletion() {
  try {
    ...
    const { InlineCompletionService } = await import('otto-core');
