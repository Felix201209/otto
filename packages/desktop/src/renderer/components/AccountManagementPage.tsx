...
export const ACCOUNT_TAG_PRESETS = [
  '普通成�?, '部门负责�?, '行政', '人事', '财务', '审批', '法务', '销�?,
  '市场', '产品', '研发', '设计', 'IT', '报修', '技术支�?, '客户支持', '采购', '数据',
] as const;

export const ACCOUNT_DEPARTMENT_PRESETS = [
  '总经�?, '人力资源�?, '财务�?, '法务�?, '销售部', '市场�?,
  '产品�?, '研发�?, '设计�?, 'IT�?, '客户成功�?,
] as const;

const ACCOUNT_TEMPLATES = [
  { id: 'member', label: '普通成�?, role: '成员', department: '', tags: ['普通成�?], isAdmin: false },
  { id: 'department-lead', label: '部门负责�?, role: '部门负责�?, department: '', tags: ['部门负责�?, '审批'], isAdmin: false },
  { id: 'it-support', label: 'IT 支持', role: 'IT 支持', department: 'IT�?, tags: ['IT', '报修', '技术支�?], isAdmin: false },
  { id: 'administrator', label: '系统管理�?, role: '系统管理�?, department: 'IT�?, tags: ['IT', '系统管理�?], isAdmin: true },
] as const;
...
export function formatInviteRemaining(expiresAt: string, now = Date.now()): string {
  ...
  if (!Number.isFinite(remaining)) return '失效时间不可�?;
  if (remaining <= 0) return '已失效，请生成新链接';
  const totalSeconds = Math.ceil(remaining / 1000);
  const hours = Math.floor(totalSeconds / 3600);
  const minutes = Math.floor((totalSeconds % 3600) / 60);
  const seconds = totalSeconds % 60;
  return `${hours} 小时 ${minutes} �?${seconds} 秒后失效`;
}
  ...
  return (
    <main className="otto-account-page">
      <header className="otto-account-hero">
        <div>
          <button type="button" className="otto-account-page__back" onClick={onBack}>�?返回工作�?/button>
        ...
        <section className="otto-account-invite" aria-label="5 小时企业引入链接">
          <header>
            <div>
              ...
              <h2>邀请成员加�?{inviteContext?.organization.name || currentAccount.organizationName}</h2>
            ...
            <button
              ...
              disabled={inviteBusy}
              aria-label={inviteContext?.invite ? '生成新引入链�? : '生成 5 小时引入链接'}
            >
          ...
          {!inviteLoading && inviteContext?.invite ? (
            <div className="otto-account-invite__body">
              ...
              <div className="otto-account-invite__link">
                ...
                <small>员工在浏览器打开后可一键唤�?Otto，首次注册时自动填入该企业邀请码�?/small>
              </div>
              <div className="otto-account-invite__actions">
                <button type="button" aria-label="复制完整引入链接" onClick={() => void copyInviteValue('link', inviteContext.invite!.link)}>{copied === 'link' ? '链接已复�? : '复制链接'}</button>
                <button type="button" aria-label="复制企业邀请码" onClick={() => void copyInviteValue('code', inviteContext.invite!.code)}>{copied === 'code' ? '邀请码已复�? : '复制邀请码'}</button>
              </div>
            </div>
          ) : null}
      ...
      <section className="otto-account-metrics" aria-label="账号概览">
        ...
        <article><span>可登�?/span><strong>{activeCount}</strong><small>{accounts.length - activeCount} 个已停用</small></article>
        <article><span>手机已登�?/span><strong>{smsCount}<i>/{accounts.length || 0}</i></strong><small>{smsCount === accounts.length && accounts.length > 0 ? '已全部登�? : '仍有账号未登记手�?}</small></article>
        <article><span>管理�?/span><strong>{adminCount}</strong><small>拥有身份管理权限</small></article>
      </section>
      ...
      <section className="otto-account-directory">
        ...
        <div className="otto-account-table" role="table" aria-label="账号列表">
          <div className="otto-account-table__row otto-account-table__header" role="row">
            <span>成员</span><span>组织信息</span><span>职责标签</span><span>访问状�?/span><span />
          </div>
          {loading ? <div className="otto-account-table__empty">正在同步企业身份目录�?/div> : null}
          {!loading && filtered.length === 0 ? <div className="otto-account-table__empty">没有匹配的成�?/div> : null}
          {filtered.map((account) => (
            <div className="otto-account-table__row" role="row" key={account.id}>
              ...
              <div><strong>{account.role || '未设置岗�?}</strong><small>{account.department || '未分配部�?}</small></div>
              ...
              <div className="otto-account-table__state">{account.isAdmin ? <span className="is-admin">管理�?/span> : <span>成员</span>}<span className={account.status === 'active' ? 'is-active' : 'is-disabled'}>{account.status === 'active' ? '可登�? : '已停�?}</span>{account.phone ? <span className="is-sms">手机</span> : null}</div>
        ...
        <div className="otto-account-editor__overlay" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !saving) setEditing(null); }}>
          <section className="otto-account-editor" role="dialog" aria-modal="true" aria-label={editing === 'new' ? '新增账号' : '编辑账号'}>
            ...
            <div className="otto-account-editor__grid">
              ...
              <label><span>手机号码</span><input aria-label="手机号码" inputMode="tel" value={draft.phone} onChange={(e) => setDraft((v) => ({ ...v, phone: e.target.value }))} placeholder="用于短信验证码登�? /></label>
              ...
              <label><span>角色</span><input aria-label="角色" value={draft.role} onChange={(e) => setDraft((v) => ({ ...v, role: e.target.value }))} placeholder="例如：桌面支�? /></label>
              <label><span>部门</span><input aria-label="部门" list="otto-account-departments" value={draft.department} onChange={(e) => setDraft((v) => ({ ...v, department: e.target.value }))} placeholder="选择或输入部�? /><datalist id="otto-account-departments">{ACCOUNT_DEPARTMENT_PRESETS.map((department) => <option key={department} value={department} />)}</datalist></label>
              ...
              {editing !== 'new' ? <label><span>账号状�?/span><select aria-label="账号状�? value={draft.status} onChange={(e) => setDraft((v) => ({ ...v, status: e.target.value as AccountDraft['status'] }))}><option value="active">可登�?/option><option value="disabled">停用</option></select></label> : null}
