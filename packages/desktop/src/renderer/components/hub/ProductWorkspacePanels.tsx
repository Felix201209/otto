...
export function OrganizationPanel({
  ...
  const [employeeScale, setEmployeeScale] = useState('1-50�?);
  ...
  if (!workspace) {
    return <Panel title="企业与身�? desc="正在读取服务端身份�?><Empty>加载中�?/Empty></Panel>;
  }
          ...
          {flow === 'owner' ? (
            <Card>
              <div className="otto-product-form">
                <label>管理者姓�?input value={managerName} onChange={(event) => setManagerName(event.target.value)} placeholder="例如：陈�? /></label>
                <label>企业名称<input value={companyName} onChange={(event) => setCompanyName(event.target.value)} placeholder="例如：北辰科技" /></label>
                <label>所属行�?input value={industry} onChange={(event) => setIndustry(event.target.value)} placeholder="例如：企业软�? /></label>
                <label>企业规模<select value={employeeScale} onChange={(event) => setEmployeeScale(event.target.value)}><option>1-50�?/option><option>51-200�?/option><option>201-500�?/option><option>500人以�?/option></select></label>
                <button
                  type="button"
        ...
        <>
          <Card>
            <div className="otto-product-identity">
              <div><span>当前身份</span><strong>{isOwner ? 'CEO · 企业管理�? : '企业成员'}</strong></div>
            ...
            <Card>
              <div className="otto-product-form">
                ...
                <p className="otto-hub__field-hint">
                  成员首次入企统一使用「企业身份控制台」生成的 5 小时企业引入链接�?                  此处链接只用于职位和总分公司组织关系，不会改变账号所属企业�?                </p>
                <label>
                  职位
                ...
                <div className="otto-product-company-accept">
                  <div className="otto-hub__field-label">输入总公�?/ 子公司签名链�?/div>
                  <textarea
                    value={companyLink}
          ...
          {state.lastInvite ? (
            <Card>
              <div className="otto-product-invite-result">
                <strong>链接已签�?· {new Date(state.lastInvite.expiresAt).toLocaleString('zh-CN')} 失效</strong>
                <textarea readOnly value={state.lastInvite.link} aria-label="生成的企业链�? />
...
export function EnterpriseModelsPanel({
  ...
  return (
    <Panel title="企业模型（未启用�? desc="当前为内部测试阶段，企业中转站、积分和充值均不参与真实运行�?>
      {!enterprise ? (
        <Empty>个人版使用你绑定的个�?API；请从对话右上角进入模型管理�?/Empty>
      ) : (
        <Empty>
          内部成员统一使用自己绑定�?API。企业中转站、托管模型、积分与充值暂不启用，也不会影响聊天请求�?        </Empty>
      )}
    </Panel>
  );
}
