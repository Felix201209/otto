...
export function CustomerModuleAuthoringDialog({
  ...
  const togglePermission = (kind: 'model' | 'http' | 'storage' | 'file'): void => {
    setTestPassed(false);
    setDraft((current) => {
      ...
      const permission = kind === 'model' ? { kind, paid: true }
        : kind === 'http' ? { kind, hosts: [], writes: false }
          : kind === 'storage' ? { kind, access: 'read-write' }
            : { kind, access: 'user-selected-read' };
      return {
        ...current,
        permissions: exists
  ...
  return createPortal(
    <div className="otto-module-marketplace-overlay">
      <div className="otto-module-marketplace" role="dialog" aria-modal="true" aria-label="创建客户模块">
        <header className="otto-module-marketplace__header">
          <div><h2>创建客户模块</h2><p>步骤 {step + 1}/6 · {STEPS[step]}</p></div>
          <button type="button" aria-label="关闭创建模块" onClick={onClose}>×</button>
        </header>
        <div className="otto-module-marketplace__catalog">
          ...
          {step === 3 ? <fieldset><legend>申请权限（安装时用户必须逐项确认）</legend>{(['model', 'http', 'storage', 'file'] as const).map((kind) => <label key={kind}><input type="checkbox" checked={draft.permissions.some((permission) => permission.kind === kind)} onChange={() => togglePermission(kind)} />{kind}{kind === 'model' ? '（可能产生费用，默认不后台运行）' : ''}</label>)}
            ...
            <p>后台执行不在首版创建向导中开放，所有付费调用只能由用户主动运行。</p>
          </fieldset> : null}
