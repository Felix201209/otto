...
/**
 * main 进程负责读取本机 profile.json，renderer 只接收已序列化数据。
 * 这里再次收窄形状与 id，避免 IPC 异常数据覆盖内置专家或污染目录。
 */
export function setAutoGeneratedAgentProfiles(
  ...
  autoGeneratedAgentProfiles = profiles.flatMap((profile) => {
    ...
    return [{
      ...
      department: null,
      skills: profile.skills.filter((skill) => typeof skill === 'string' && skill.trim()),
      systemPrompt: profile.systemPrompt,
    }];
  });
}

/** 个人版目录：基础 Otto + 统一会议 Agent + 7 位其他通用专家 + 自动孵化的 Skill 专家。 */
