...
const ENTERPRISE_CEO_AGENT_PROFILES: readonly AgentProfile[] = [
...
/** All distinct UI profiles; each identity-specific view still exposes exactly 9. */
...
let autoGeneratedAgentProfiles: AgentProfile[] = [];

/**
 * main 进程负责读取用户确认生成的 profile.json；renderer 只接收序列化数据。
 * 这里再次限制 id、必填字段和内置 id 冲突，避免损坏文件污染专家目录。
 */
export function setAutoGeneratedAgentProfiles(
  profiles: readonly AutoGeneratedAgentProfileInput[],
): void {
  const reservedIds = new Set(ALL_AGENT_PROFILES.map((profile) => profile.id));
  const acceptedIds = new Set<string>();
  autoGeneratedAgentProfiles = profiles.flatMap((profile) => {
    ...
    return [{
      id,
      name,
      tagline: profile.tagline.trim() || '自动孵化的工作专家',
      scope: profile.scope === 'base' ? 'base' : 'personal',
      department: null,
...
export function getPersonalAgentProfiles(): readonly AgentProfile[] {
  return [...BASE_AGENT_PROFILES, ...autoGeneratedAgentProfiles];
}
...
export function getEnterpriseAgentProfiles(
  ...
  const identityProfiles = role === 'company_owner' || role === 'company_admin'
    ? ENTERPRISE_CEO_AGENT_PROFILES
    : ENTERPRISE_WORK_AGENT_PROFILES;
  return [...identityProfiles, ...autoGeneratedAgentProfiles];
}
