...
export function getPersonalAgentProfiles(): readonly AgentProfile[] {
  const base = [...BASE_AGENT_PROFILES];
  try {
    // webpack alias 会解析 otto-core；浏览器模式 loadAutoGeneratedProfiles 返回 []
    const { loadAutoGeneratedProfiles: loadProfiles } = require('otto-core');
    const autoProfiles: Array<{
      id: string; name: string; tagline: string; scope: string;
      department: string | null; skills: string[]; systemPrompt: string;
    }> = loadProfiles();
    for (const ap of autoProfiles) {
      base.push({
        ...
        name: ap.name,
        tagline: ap.tagline,
        scope: (ap.scope as AgentProfileScope) || 'personal',
        department: null,
        skills: ap.skills || [],
        systemPrompt: ap.systemPrompt,
      });
    }
  } catch { /* otto-core not available (browser mode) */ }
  return base;
}
