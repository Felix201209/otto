# Otto Enterprise v1.8.9 新服务器迁移包
...
- 这是“当前服务器原样迁入新机器”的包，数据导入只接受当前生产 schema v2；旧 schema 必须先在旧服务器走单独的受控升级，不能把迁入流程顺手当升级流程。
...
```bash
sha256sum -c otto-enterprise-oneclick-v1.8.9-*.tar.gz.sha256
```
...
```bash
tar -xzf otto-enterprise-oneclick-v1.8.9-*.tar.gz
cd otto-enterprise-oneclick-v1.8.9-*
```
