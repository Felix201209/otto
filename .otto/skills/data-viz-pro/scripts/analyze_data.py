...
def _import_pandas():
    try:
        ...
        _die("pandas is required. Install with: python -m pip install pandas openpyxl")
...
def _bar_by_category_chart(df, cat_col, num_col):
    grouped = df.groupby(cat_col, observed=True)[num_col].mean().sort_values(ascending=False).head(10)
    if grouped.empty:
        return None
...
def _hist_chart(df, num_col):
    values = [_round(v, 2) for v in df[num_col].dropna().tolist()]
    if len(values) < 5:
        return None
    return {
        "type": "histogram",
...
def write_outputs(input_path: Path, output_dir: Path, render: bool):
    ...
    for index, (name, cfg) in enumerate(charts, start=1):
        ...
        if render:
            ...
            subprocess.run([sys.executable, str(script), str(cfg_path), str(png_path)], check=False)
